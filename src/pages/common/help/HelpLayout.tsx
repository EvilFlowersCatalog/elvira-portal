...
const HelpLayout = ({ title, toc, children }: HelpLayoutProps) => {
  ...
  return (
    <div className="h-screen overflow-auto">
      ...
      {/* Hero header */}
      <div className="relative bg-primaryLight dark:bg-strongDarkGray overflow-hidden flex flex-col items-center justify-end pb-6 pt-10" style={{ minHeight: 160 }}>
      ...
      <div className="px-6 py-8 max-w-5xl mx-auto flex gap-6 items-start">
        {/* Main article */}
        <div className="flex-1 bg-white dark:bg-strongDarkGray rounded-lg p-6 min-w-0">
          {children}
        </div>
          ...
          <div className="rounded-lg p-4">
            {toc.map((section) => (
              <div key={section.heading} className="mb-6 last:mb-0">
                <p className="text-xs font-bold text-secondary dark:text-secondaryLight mb-3">{section.heading}</p>
                <ul className="space-y-1.5">
                  {section.items.map((item) => (
                    <li key={item.id} className="text-xs text-gray-600 dark:text-zinc-400 flex items-baseline gap-1.5">
                      <span className="shrink-0">•</span>
                      <button
                        className="text-left hover:text-primary dark:hover:text-primary transition-colors cursor-pointer"
