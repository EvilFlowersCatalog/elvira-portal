...
const EntryDetail = ({ triggerReload }: IEntryDetailParams) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (entryDetail.response.config.readium_enabled) {
          const licenses = await getLicenses({
            page: 1,
            limit: 1,
            entry_id: entryDetail.response.id,
            user_id: auth?.userId,
            state: LICENSE_STATE.active,
          });
          setLicense(licenses.items[0]);
        }
  ...
  return (
    <>
      {showLicenseForm && (
        <LicenseForm setOpen={setShowLicenseForm} entryId={entryId!} />
      )}
      <div
        className={`fixed top-0 right-0 z-40 h-full w-full md:w-2/4 lg:w-2/5 xl:w-1/4 bg-darkGray bg-opacity-95 flex flex-col p-4 overflow-auto`}
      >
        <button
          ...
          <div className={'flex-1 flex flex-col'}>
            <div className={'flex flex-col gap-2 justify-center items-center'}>
              ...
              <PDFButtons
                ...
                license={license}
                readium={entry.response.config.readium_enabled}
                showLicenseForm={setShowLicenseForm}
              />
