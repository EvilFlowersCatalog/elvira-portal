...
const EntryDetail = ({ triggerReload }: IEntryDetailParams) => {
  ...
  const askAi = () => {
    ...
    params.set('assistant-entry-id', entryId || '');
    ...
    >
      {!entry ? (
        ...
        <div className={'flex h-full flex-col mdlg:flex-row'}>
          ...
          <div className='p-4 pt-8 bg-white dark:bg-darkGray mdlg:overflow-y-auto h-full pb-20 w-full'>
            <DetailHeader
              entry={entry}
                ...
                <div className="flex flex-col gap-1">
                  {entry.categories.length === 0 ? (
                    ...
                    entry.categories.map((category, index) => (
                      <span
                        key={index}
                        className="w-fit text-secondary dark:text-secondaryLight font-extrabold cursor-pointer"
                        onClick={() => {
